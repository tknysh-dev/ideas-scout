...
# ---------------------------------------------------------------------------
# дефект 34: timestamp для RUN_ID у runner.sh має братись у UTC (date -u), а
# не в локальному часі — інакше run_id сортуються інакше, ніж started_at/
# finished_at у БД (ті завжди UTC). build_run_id сама чиста й не викликає
# date, тож перевіряємо джерело виклику в runner.sh статично.
# ---------------------------------------------------------------------------

# shellcheck disable=SC2016  # це grep-патерн, не bash-підстановка
expect_ok "runner.sh: RUN_ID будується з date -u (UTC), не локального часу" \
  grep -q 'build_run_id "$(date -u ' "$SCRIPT_DIR/runner.sh"
