...
# "перевернутий" діапазон (start > end): ФАКТИЧНА поведінка — функція просто
# порівнює now_min з sm/em як звичайні числа, тому now_min >= sm ("19:00-09:00" ->
# sm=1140) і now_min < em (=540) не можуть виконатись одночасно ні для якого
# часу доби. Результат: захист мовчки НІКОЛИ не спрацьовує (завжди "поза
# вікном" -> runner.sh прогін ніколи не пропускає) — це виглядає як діра,
# не як "вікно через північ", див. звіт.
expect_fail "перевернутий діапазон 19:00-09:00: полудень — 'поза вікном'" \
  within_work_window 3 12 00 "1,2,3,4,5" "19:00-09:00"
expect_fail "перевернутий діапазон 19:00-09:00: 20:00 — теж 'поза вікном'" \
  within_work_window 3 20 00 "1,2,3,4,5" "19:00-09:00"
expect_fail "перевернутий діапазон 19:00-09:00: 02:00 ночі — теж 'поза вікном'" \
  within_work_window 3 02 00 "1,2,3,4,5" "19:00-09:00"
expect_fail "перевернутий діапазон 19:00-09:00: 00:00 — теж 'поза вікном'" \
