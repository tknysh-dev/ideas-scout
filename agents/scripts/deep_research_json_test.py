...
переважує справжнє закриття. _balance_brackets тепер теж рахує глибину (через
стек дужок і облік ком по глибині) при виборі, де відрізати недописаний запис
на обриві всередині рядка — раніше різав по останній комі в УСЬОМУ тексті, що
на певних обривах (коли в недописаному записі вже була своя внутрішня кома)
давало невалідний JSON. Якщо межі з попереднім елементом на тій самій глибині
немає (обрив стався в першому елементі контейнера), код свідомо лишає стару
поведінку як є — і невалідний результат тут є навмисним запобіжником,
на якому спирається test_backtick_inside_quoted_string_still_recovered:
кандидат, обрізаний по бектику в рядковому значенні, має провалитись, щоб
черга кандидатів дійшла до кращого ("увесь текст").
"""
import json
import unittest
...
class BalanceBrackets(unittest.TestCase):
    ...
    def test_cut_mid_string_respects_nesting_not_last_comma_in_text(self):
        # РАНІШЕ (ВІДОМИЙ ДЕФЕКТ, "СУМНІВНО"): коли рядок обривається
        # всередині значення, код різав по ОСТАННІЙ комі в усьому тексті
        # (raw.rfind(","), без огляду на глибину вкладеності). Якщо в
        # недописаному записі вже була своя внутрішня кома (тут — між
        # "criterion_key" і "verdict" другого запису), різ відбувався ПІСЛЯ
        # неї, а не на межі записів масиву — результат був синтаксично
        # невалідним JSON (другий запис лишався без власної закривної "}").
        # Тепер відрізається до позиції, де недописаний запис ПОЧАВСЯ (за
        # стеком дужок), тож внутрішня кома незавершеного запису більше не
        # плутається з межею елементів масиву.
        text = '{"a": [{"criterion_key": "0"}, {"criterion_key": "1", "verdict": "fail'
        result = dr._balance_brackets(text)
        self.assertEqual(result, '{"a": [{"criterion_key": "0"}]}')
        self.assertEqual(json.loads(result), {"a": [{"criterion_key": "0"}]})

    def test_cut_mid_string_on_first_incomplete_record_keeps_old_fallback(self):
        # Межовий випадок нового фіксу: обрив усередині ПЕРШОГО (і єдиного)
        # запису масиву — на цій глибині немає межі з попереднім елементом
        # (сусіда немає), тож респектувати нема що: код свідомо лишає стару
        # поведінку "по останній комі в усьому тексті", яка тут так само, як
        # і до фіксу, дає невалідний JSON. Рятувати за рахунок ВИГАДУВАННЯ
        # межі (наприклад, тихо відкидати єдиний запис і лишати порожній
        # масив) навмисно НЕ стали: саме на цьому спирається
        # test_backtick_inside_quoted_string_still_recovered нижче — там
        # перший (обрізаний по бектику) кандидат МАЄ провалитись, щоб чергу
        # кандидатів дійшла до кращого ("увесь текст").
        text = '{"criteria": [{"criterion_key": "0", "verdict": "fail'
        result = dr._balance_brackets(text)
