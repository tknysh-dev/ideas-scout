...
# Порядок перевірок навмисний (дефект 21): літерал, що ВИГЛЯДАЄ як JSON (перший
# непробільний символ { або [), трактується як літерал ДО перевірки [ -f "$arg" ]
# — інакше payload, який випадково збігається з іменем наявного файлу в cwd,
# мовчки читався б з диска замість передачі як є.
_json_arg() {
  local arg="${1:-}"
  if [ "$arg" = "-" ]; then
    cat
