...
# документована особливість контракту: якщо ЛІТЕРАЛЬНИЙ рядок JSON випадково
# збігається з іменем наявного файлу в поточній директорії, _json_arg прочитає
# файл, а не поверне рядок як літерал. Виклики db.sh завжди передають або
# справжній JSON (не є шляхом), або "-", або свідомий шлях — тому на практиці
# не зламано, але контракт саме такий: [ -f "$arg" ] перевіряється РАНІШЕ, ніж
# рядок трактується як літерал.
( cd "$DBTEST_TMP" && printf '%s' '{"from":"file"}' > '{"literal":true}' )
expect_eq "_json_arg: збіг літералу з іменем наявного файлу читає файл, не літерал" \
  '{"from":"file"}' "$(cd "$DBTEST_TMP" && _json_arg '{"literal":true}')"
