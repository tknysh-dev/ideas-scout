...
const JOB_HANDLERS = Object.freeze({
  infrastructure_dry_run: join(REPO_ROOT, "agents/scripts/infrastructure-dry-run.sh"),
});

export function buildRunId(jobId, date = new Date()) {
  const timestamp = date.toISOString().replace(/[-:TZ.]/g, "").slice(0, 14);
  return `${timestamp}-local-infrastructure-dry-run-${jobId.slice(0, 8)}`;
}

export function commandForJob(job) {
  const executable = JOB_HANDLERS[job.type];
  if (!executable) throw new Error(`Непідтримуваний тип job: ${job.type}`);
...
export function createJobWorker({ supabase, workerId }) {
  ...
  async function executeJob(job) {
    ...
    const runStatus = success ? "dry_run" : "error";
    ...
    const { error: runFinishError } = await supabase
      ...
      .update({
        ...
        errors,
        notes: success
          ? "Інфраструктурний dry run успішно виконано."
          : "Інфраструктурний dry run завершився помилкою.",
        meta: {
          job_id: job.id,
          worker_id: workerId,
