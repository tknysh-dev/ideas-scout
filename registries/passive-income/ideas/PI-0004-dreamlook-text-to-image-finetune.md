...
rejection_detail: "Файнтюн і інференс дифузійних моделей вимагають GPU-інфраструктури (тренування SDXL-кроків, генерація зображень), якої немає в `ai-capabilities.md` і яку не покривають M1/M5 + підписки Claude/Codex. Хмарні GPU під навантаженням — регулярні витрати, що суттєво перевищують поріг €100 без погодження. Це одночасно брак можливості (CAPABILITY_GAP) і капітальний бар'єр."
...
**2. Легальність і податковий статус — гейт (park).** Успадковує гейт PI-0001. Не фатально на цьому рівні. (Додатковий, не оцінюваний тут ризик: правовий статус навчальних даних/вихідних зображень у генеративному AI — сіра зона, яку варто підняти, якщо ніша колись повернеться до аналізу.)

**3. Реалізовність наявними засобами — FATAL → `CAPABILITY_GAP`.** Перший фатальний провал (гейт 2 — park). Файнтюн дифузійних моделей (1500 SDXL-кроків за ~10 хв на run) і генерація зображень вимагають GPU-обчислень, яких: (а) немає в `ai-capabilities.md`; (б) не дають M1/M5 у продакшн-обсязі; (в) не покривають підписки Claude Code / Codex (це LLM-агенти для коду, не GPU-хостинг). Хмарний GPU під клієнтським навантаженням — регулярна витрата, що явно перевищує €100/міс. Це і брак можливості, і капітальний бар'єр — за v0.1 кодуємо як `CAPABILITY_GAP` (брак можливості первинний; сам GPU можна орендувати, але тільки понад поріг капіталу й без наявної абстракції). Зупинка чек-листа тут; критерії 4–5 не оцінювались.
