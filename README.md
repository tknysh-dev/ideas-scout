...
`./check.sh` — одна команда на всі статичні перевірки й тести: синтаксис і
`shellcheck` усіх shell-скриптів, компільованість і `ruff` для python-скриптів
агентів, типи/lint/тести порталу і тести воркера. Як і `doctor.sh`, вона не
спиняється на першій помилці, а проганяє все й доповідає підсумок.

```bash
./check.sh
```

`./check.sh --fast` пропускає єдине по-справжньому повільне — `tsc --noEmit` і
`npm run lint` у `dashboard/`. Тести й перевірки скриптів лишаються.
