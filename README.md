...
Хуки поділені так, щоб коміт лишався дешевим:

- `hooks/pre-commit` — лише `./check.sh --secrets` (частки секунди). Усе інше
  можна полагодити наступним комітом, а ключ, що потрапив в історію, звідти
  вже просто так не прибрати.
- `hooks/pre-push` — повний `./check.sh`: тести, типи, lint, збірка, сторожі.
  Зупиняємось на межі, за якою зміни бачить не тільки ця машина.

Git не переносить хуки при клонуванні, тому на кожній новій машині їх треба
увімкнути один раз:
...
Разово обійти — `git commit --no-verify` і `git push --no-verify`.
