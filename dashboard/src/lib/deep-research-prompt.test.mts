...
// index===0 — теж дійсний збіг (перевіряти треба на undefined, а не на falsy):
// коли "---" стоїть у першому рядку без жодного тексту перед ним, роздільник
// усе одно знімається.
test("роздільник на самому початку шаблону знімається так само, як і в середині", () => {
  const prompt = renderHandoffPrompt({
    idea,
    sources: [],
    template: "---\nТіло {{IDEA_ID}}",
  ...
  assert.match(prompt, /^Тіло PI-0013/);
});
