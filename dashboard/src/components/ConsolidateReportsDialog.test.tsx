...
describe("ConsolidateReportsDialog — консолідація", () => {
  async function previewOk() {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Розібрати текст" }));
    await screen.findByText(/Розпізнано/);
    // Чекаємо саме на кнопку, а не лише на текст поруч: під конкурентним
    // рендером React 19 «Розпізнано» встигає зʼявитись раніше, ніж кнопка
    // комітиться, і синхронний getByRole у викликах нижче зрідка її не бачив.
    await screen.findByRole("button", { name: "Запустити консолідацію" });
  }
